...
curl -fsSL https://github.com/mfactory-lab/sv-manager/archive/refs/heads/develop.zip --output sv_manager.zip
echo "Unpack"
unzip sv_manager.zip -d .

cd ./sv_manager_develop
...
ansible-playbook --connection=local --inventory ./inventory --limit local  pb_install_monitoring.yaml -e validator.name=$VALIDATOR_NAME -e validator.secrets_path=$PATH_TO_VALIDATOR_KEYS -e cluster.rpc_address=$entry_point
